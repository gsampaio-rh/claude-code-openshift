...
5 sprints de 1 semana cobrindo Fases 0-8 do PRD + sprints de exploração (A-E). Fase 9 (Dev Spaces) eh pos-PoC.

```
Sprint 1 ████░░░░░░░░░░░░░░░░ Infra + vLLM + Claude Code standalone
Sprint 2 ░░░░████████░░░░░░░░ Observability + Safety + Coder + UI/Multi-Agent  ← CURRENT
...
- [x] Executar script de validacao completo: `inference/vllm/scripts/02-validate-model.sh` — 21 checks pass, 1 warning
...
- [x] **Observabilidade de inferencia (vLLM)** — Sprint A de [FUTURE_EXPLORATIONS.md](FUTURE_EXPLORATIONS.md):
  ...
  - [x] Criar ServiceMonitor pra vLLM (`inference/vllm/manifests/servicemonitor.yaml`) — scrape 97 metricas nativas a cada 15s
...
### 2.1.2 UI, Multi-Agent e Referências da Comunidade

> **Referência:** [FUTURE_EXPLORATIONS.md §Sprint E](FUTURE_EXPLORATIONS.md)

#### UI para Claude Code

**Problema:** Operadores e devs enterprise precisam de UI para monitorar sessões ativas, visualizar traces, inspecionar o que o agente está fazendo, e gerenciar sessões. O CLI desde v2.1.20 esconde detalhes (`Read 3 files`, `Edited 2 files` — sem paths, sem diffs), e o `--verbose` despeja JSON cru ilegível.

**claude-devtools** — [github.com/matt1398/claude-devtools](https://github.com/matt1398/claude-devtools) (3.1k stars, MIT, Electron + Docker)

Lê `~/.claude/` (logs de sessão) e reconstrói context, compaction, tool calls e subagent trees sem wrapper ou API keys.

- [x] Build in-cluster via BuildConfig no registry interno (ADR-023) (`agents/claude-code/manifests/devtools-build.yaml`)
- [x] Sidecar no pod standalone (`agents/claude-code/manifests/standalone-pod.yaml`) — monta `~/.claude/` read-only
- [x] NetworkPolicy para acesso via Route (ingress controller)
- [ ] Validar context reconstruction (atribuição de tokens por turno em 7 categorias)
- [ ] Validar compaction visualization (momento exato de compactação + o que foi perdido)
- [ ] Validar tool call inspector (diffs inline, syntax highlighting, output de bash)
- [ ] Validar subagent trees (árvores de execução recursivas com tokens, duração, custo)
- [ ] Testar multi-session side-by-side para cenários multi-agent

**opcode** — [github.com/winfunc/opcode](https://github.com/winfunc/opcode) (21.5k stars, AGPL-3.0, Tauri 2 + React 18 + Rust + SQLite) — **ABANDONADO**

GUI desktop completa para Claude Code. Possui web server mode (Axum), mas requer acesso ao binário `claude`, diretórios de projetos, e controle de processos — incompatível com sidecar containerizado. Build e deploy no cluster foram validados (Rust compilou em 5min, imagem funcional, HTTP 200), mas features essenciais (CLAUDE.md, instalação detection, session management) não funcionam sem co-localização com o agente.

- [x] Build Dockerfile multi-stage (Rust + frontend) e deploy como sidecar — funcional
- [x] Validar no cluster — HTTP 200, session browser parcialmente funcional
- [x] **Decisão: abandonar** — app desktop, incompatível com arquitetura de containers separados

**claude-code-hooks-multi-agent-observability** — [github.com/disler/claude-code-hooks-multi-agent-observability](https://github.com/disler/claude-code-hooks-multi-agent-observability) (1.4k stars, Vue 3 + Bun + SQLite + WebSocket) — **ABANDONADO**

Deploy e pipeline de hooks funcionaram — capturou 6 tipos de evento em 3 sessões via sidecar Bun + send_event.py stdlib. Porém a UI Vue é pobre: layout desorganizado, sem polimento visual, difícil de usar como dashboard de produção. A arquitetura de hooks (stdin JSON → HTTP POST → SQLite → WebSocket) é boa referência, mas o frontend não atende.

- [x] Deploy servidor (Bun) + client (Vue) como sidecar no cluster — funcional
- [x] Configurar hooks no `.claude/settings.json` — 12 eventos capturados via `send_event.py` stdlib-only
- [x] Validar pipeline end-to-end — eventos com tool_name, inputs, outputs, session tracking
- [x] **Decisão: abandonar** — UI inadequada para uso enterprise

**agent-flow** — [github.com/patoles/agent-flow](https://github.com/patoles/agent-flow) (665 stars, Apache-2.0, Next.js + TypeScript, extensão VS Code) — **ABANDONADO**

Visualização em tempo real da orquestração de agentes como grafo interativo de nós. Testado como sidecar no cluster: build funcionou, UI acessível, mas a arquitetura de hooks usa discovery-based port (porta aleatória via ficheiro JSON em `~/.claude/agent-flow/`), incompatível com sidecar networking. Fixar porta requer patches invasivos no source. A complexidade de adaptação não justifica o benefício vs agents-observe.

- [x] Build in-cluster via BuildConfig com patches (0.0.0.0 binding, fixed port)
- [x] Deploy como sidecar no pod do agente — UI funcional, hook server instável
- [x] **Decisão: abandonar** — discovery-based hook architecture incompatível com sidecar, complexidade de patches excessiva

**agents-observe** — [github.com/simple10/agents-observe](https://github.com/simple10/agents-observe) (435 stars, MIT, React 19 + Node/Hono + SQLite + Docker) — **ADOTADO** ✔

Dashboard de observabilidade para Claude Code via hooks. Arquitetura: `Claude Hooks → send_event.sh → HTTP POST → SQLite → WebSocket → React Dashboard`. Hierarquia de subagentes, human-readable session names, tool dedup (Pre+Post merged), filtro por agent/tool/search. Upstream requer patch para WSS em HTTPS (ADR-022, ADR-023). Inicialmente deployado como sidecar (ADR-022), **decoupled para Deployment standalone** (ADR-024) — lifecycle independente, sem restart do agente para iterar.

- [x] Build in-cluster via BuildConfig com patch WSS (`agents/claude-code/manifests/agents-observe-build.yaml`)
- [x] ~~Sidecar no pod standalone~~ → Deployment standalone com Service + Route edge TLS (`agents/claude-code/manifests/agents-observe.yaml`) (ADR-024)
- [x] Hook scripts (`send_event.sh`) — 12 eventos capturados, POST síncrono via Kubernetes service DNS
- [x] Pipeline end-to-end validado — 65+ eventos, até 4 subagentes simultâneos, 5 sessões
- [x] WebSocket corrigido para wss:// via patch no build (mixed content fix)
- [x] NetworkPolicy configurada para comunicação inter-pod na porta 4981
- [x] Bug fix: `send_event.sh` background subshell matava processo antes do HTTP completar — corrigido para execução síncrona com `process.exit(0)`
- [ ] Validar hierarquia de subagentes no dashboard (parent/child tracking visual)
- [ ] Testar com sessões longas (estabilidade do SQLite ephemeral)
- [ ] Avaliar persistência de eventos via PVC (atualmente `/tmp`, perde dados no restart)

**Outros projetos de referência (não testados):**

- [Claude-Code-Agent-Monitor](https://github.com/hoangsonww/Claude-Code-Agent-Monitor) — Node.js + Express + React + WebSockets, Kanban + analytics

#### Multi-Agent Orchestration

**Problema:** Um único agente resolve tarefas isoladas. Workloads reais precisam de múltiplos agentes colaborando — code review, testes, refactoring. Como orquestrar isso no cluster?

- [x] Habilitar `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` no ConfigMap
- [x] Testar execução multi-agent com divisão de tarefas — até 5 subagentes simultâneos, 40 eventos em uma sessão
- [ ] Comunicação inter-agente (shared context, message passing)
- [ ] Scheduling e priorização de agentes no cluster
- [ ] Isolamento por agente (cada um no seu pod/microVM)
- [ ] Observabilidade cross-agent (traces distribuídos, custo agregado)
- [ ] Avaliar projetos da comunidade para referência:
  - [Aperant](https://github.com/AndyMik90/Aperant) — Electron, até 12 agentes em paralelo em git worktrees isolados
  - [claude_code_agent_farm](https://github.com/Dicklesworthstone/claude_code_agent_farm) — 20+ agentes em paralelo com coordenação por locks
  - [claude-code-workflow-orchestration](https://github.com/barkain/claude-code-workflow-orchestration) — Plugin multi-step com delegação especializada
  - [gastown](https://github.com/steveyegge/gastown) — Gerenciador de workspace multi-agente
  - [harness](https://github.com/revfactory/harness) — Meta-skill que projeta equipes de agentes específicas por domínio

#### Model upgrade: gpt-oss-20b

- [x] Deploy vLLM com modelo `openai/gpt-oss-20b` (`inference/vllm/manifests/gpt-oss-20b-deployment.yaml`)
- [x] Atualizar ConfigMap do agente: `ANTHROPIC_BASE_URL`, model tiers, context window
- [x] Validar E2E: Claude Code → vLLM (gpt-oss-20b) → código funcional — subagentes geraram código, testes passaram
- [ ] Benchmark: comparar latência e qualidade vs Qwen 2.5 14B
...
| G2.9 | claude-devtools acessível via Route com sessão visível | Browser |
| G2.10 | Context reconstruction mostra breakdown de tokens por turno | UI do devtools |
| G2.11 | Multi-agent teams habilitado e funcional | `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` + teste com 2+ agentes |
| G2.12 | gpt-oss-20b servindo e agente conversando | E2E test |
| G2.13 | Projetos de referência avaliados e documentados | Notas em FUTURE_EXPLORATIONS.md |
...
flowchart LR
    ...
    S2 -->|"MLflow + Guardrails + Coder + UI/Multi-Agent"| S3
    S3 -->|"SPIFFE tokens"| S4
